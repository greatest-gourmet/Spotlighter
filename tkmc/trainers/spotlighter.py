...
class Memory(nn.Module):
    ...
    def  imageread(self,x):   #加工图片   ,还是当image_token处理可能会好一点     ,这里处理的是教师模块给的数据
     ...
     #   base_features= softmax_attention_cache.T@base_features
        ...
        base_features=softmax_attention_cache@self.text_fine_cache   
...
class CustomCLIP_memory(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        self.image_encoder = clip_model.visual
    ...
    def forward(self, image, label=None):
        ...
        return image_features, logit_scale, image_fine_list, top_image_fine_list,top_image_top_fine_list,top_image_low_fine_list
...
class PromptKD(TrainerX):
    ...
    def build_model(self):
        ...
        self.model = CustomCLIP_memory(cfg, classnames, clip_model)
       # self.model_memory = CustomCLIP_memory(cfg, classnames, clip_model)
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            image_ft, logit_scale, image_fine, top_image_fine ,top_top_image_fine,top_low_image_fine= model(image, label)
            ...
            top_top_stu_logits=logit_scale * top_top_image_fine @ tea_text_features.t()
            top_low_stu_logits=logit_scale * top_low_image_fine @ tea_text_features.t()
            L_ukd = F.kl_div(
            ...
            L_sem_top = F.kl_div(
                F.log_softmax(top_top_stu_logits / self.temperature, dim=1),
                F.softmax(tea_logits / self.temperature, dim=1).repeat_interleave(repeats = 4 ,dim = 0),
                reduction='sum',
            ) * (self.temperature * self.temperature) / top_top_stu_logits.numel()  # 求平均

            L_sem_low = F.kl_div(
                F.log_softmax(top_low_stu_logits / self.temperature, dim=1),
                F.softmax(tea_logits / self.temperature, dim=1).repeat_interleave(repeats = 5 ,dim = 0),
                reduction='sum',
            ) * (self.temperature * self.temperature) / top_low_stu_logits.numel()  # 求平均
       
            
            loss = self.cfg.TRAINER.PROMPTKD.KD_WEIGHT * L_ukd + self.cfg.TRAINER.PROMPTKD.KD_WEIGHT / 100 * L_sem + \
                                                                 self.cfg.TRAINER.PROMPTKD.KD_WEIGHT / 1000 * L_sem_top +\
                                                                 self.cfg.TRAINER.PROMPTKD.KD_WEIGHT / 1000 * L_sem_low +\
                                                                self.cfg.TRAINER.PROMPTKD.KD_WEIGHT / 50 * L_mem+ \
                                                                 self.cfg.TRAINER.PROMPTKD.KD_WEIGHT/100*L_img    
        ...
        loss_summary = {"loss": loss.item(),
                        ...
                        "L_sem_top": self.cfg.TRAINER.PROMPTKD.KD_WEIGHT / 1000 * L_sem_top.item(),
                       "L_sem_low": self.cfg.TRAINER.PROMPTKD.KD_WEIGHT / 1000 * L_sem_low.item(),
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            image_ft, logit_scale, image_fine, _,_,_  = self.model(image, label)
